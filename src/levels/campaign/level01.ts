...
export function buildLevel01(board: Board): Level {
  const b = new LevelBuilder(board, 101, { name: 'campaign.level0.name', difficulty: 1, archetype: 'serpentine', tune: { hpMul: 1.40 } })
  b.path([
  ...
  // Band A (rows 0–1, above the top lane): the power-supply rail + a decoupling pair
  b.block(powerSupply([1, 0], b.alloc))        // J→D→C→U→C·C across the top
  b.block(passiveBank([18, 0], 4, b.alloc))    // decoupling pair, top-right
  // Band B (rows 3–5, between lanes 1 & 2): a resistor divider
  b.block(passiveBank([1, 3], 5, b.alloc))     // R-divider, left
  // Band D (rows 7–9, between lanes 2 & 3): an op-amp stage + an LED indicator
  b.block(opAmp([6, 7], b.alloc))              // SOIC + feedback Rs (bypass cap rides up into band B)
  b.block(ledIndicator([14, 8], b.alloc))      // LED + series R
  // Band F (rows 11–13, between lanes 3 & 4): crystal oscillator + an RC filter
  b.block(passiveBank([6, 11], 7, b.alloc))    // crystal + 2 load caps
  b.block(passiveBank([15, 11], 0, b.alloc))   // RC filter
