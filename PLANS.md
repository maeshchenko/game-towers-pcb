...
- **Этап 2: juice-пас** — GSAP-твины (постройка/апгрейд/продажа), хит-фидбек (вспышка,
  ...
  (bloom на слои, RGBSplit-транзиенты, CRT), событийная пунктуация (виньетка базы, баннер
  волны, slam звёзд). Живая плата: SHOW_DECOR=true + пульсы по трассам (`TracePulse`) готовы;
  LED-мигание в декоре осталось в бэклоге (нет анимации у `led5mm` вовсе, не только из-за кэша).
  ...
  поверх врагов); `particles` → нативный ParticleContainer.
